...
cd "$REPO_DIR" || { echo "❌ ディレクトリが見つかりません: $REPO_DIR"; exit 1; }

# コミットメッセージ（引数がなければ日時を使用）
MSG="${1:-$(date '+%Y-%m-%d %H:%M') - 進捗更新}"

echo "📁 BODY MASTER — GitHub プッシュ開始"
echo "📝 コミットメッセージ: $MSG"
echo ""
...
echo "🔗 https://github.com/akiratakahashi929-cloud/body-master"
